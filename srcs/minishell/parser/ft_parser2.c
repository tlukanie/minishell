...
static void	ft_fillct_input(t_ct *ct, char *file, int *r)
{
	if (*r & INFILE)
	{
...
static void	ft_fillct_output(t_ct *ct, char *file, int *r)
{
	if (*r & OUTFILE)
	{
		if (!access(file, W_OK) || access(file, F_OK))
...
{
	ft_fillct_input(ct, file, r);
	ft_fillct_output(ct, file, r);
	if (*r & ERRFILE)
	{
		if (!access(file, W_OK) || access(file, F_OK))
